import { type App, Modal, Setting, sanitizeHTMLToDom, Notice, setIcon } from "obsidian"
...
export class SettingModal extends Modal {
    onOpen() {
        const { contentEl } = this
        contentEl.setText(sanitizeHTMLToDom(`Hm, that doesn't seem right. Make sure to have your colors in the following format:<br><br>[#hexcolorcode, #hexcolorcode, ... etc]`))
    }
...
export class PaletteModal extends Modal {
    ...
    onSubmit: () => void
    constructor(app: App, plugin: MyPlugin, onSubmit: () => void, color_id?: string) {
        super(app)
        this.plugin = plugin
    ...
    onOpen() {
        ...
        new Setting(contentEl)
            ...
            .addButton(btn => {
                btn.setClass("pickr")
                ;(document.querySelector("button.pickr") as HTMLElement).style.setProperty("background", color)
                ;(document.querySelector("button.pickr") as HTMLElement).setAttribute("style", `background: ${color}; padding: var(--size-4-4);`)

                const p = initpickr(color, "div.modal-container:last-child")
                p.setColor(color)
                p.on("change", (c: Pickr.HSVaColor) => {
                    ;(document.querySelector("button.pickr") as HTMLElement).style.setProperty("background", `${c.toHEXA().toString()}`)
        ...
        new Setting(contentEl).addButton(btn =>
            btn.setButtonText("Submit").onClick(() => {
                for (const l of document.querySelectorAll('li span[class="hex-code"]')) {
